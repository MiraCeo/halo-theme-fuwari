...
音乐配置的交互重做，把原本需要进弹窗的设置全部搬到设置页上：

- **自定义曲目改为「音乐」标签页内的直接编辑**。此前曲目是一个可增删列表，每次编辑都要点「编辑条目」打开弹窗；现在改为若干固定曲目槽位，音频、标题、封面、歌词都在子菜单里直接填写。
- **音频与封面使用附件控件**，与基础设置的 Banner 图片一致：上传、从附件库选择、粘贴链接、删除。
- **标题留空时自动取音频文件名**（去扩展名、去查询串、解码百分号转义）。不读取 ID3 标签——附件字段只提供地址，无法取得文件内容。
- 歌词为多行文本域，一行一句；行首带 `[分:秒.百分秒]` 时跟随播放高亮，不带也可整段显示。
- **Meting API 增加「启用」开关**，开启后才展开接口地址、平台、类型与 ID。此前靠"接口地址留空即不使用"的隐式约定，既无法表达"想停用但保留地址"，也容易只填地址漏填 ID。
- 音乐总开关未开启时不再加载任何音乐界面。

> **升级说明**：本版本改变了音乐配置的存储结构，旧的自定义曲目配置不会迁移，请在「音乐」标签页重新填写。若此前配置过音乐，请同时确认侧边栏部件列表中已无残留的音乐部件（该选项自 v2.1.4 起已移除）。
