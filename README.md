...
这是一个紧急修复版本，修复 v2.1.4 导致页面无法渲染（白屏）的问题：

- **移除 v2.1.4 顶栏音乐按钮中 Halo 模板引擎不支持的表达式**。该版本使用了列表转换方法和集合选择表达式，Halo 无法求值，而模板是服务端渲染的——表达式出错会让**整个页面**渲染失败，而不是让按钮降级消失。
- 顶栏按钮改为最朴素的写法：服务端只做一次普通遍历，可见性、开关判断和"是否真的有播放器"全部由客户端决定。按钮默认隐藏，只有确认存在可控制的播放器后才显示。
- 顶栏按钮由 `id` 改为类选择器。此前 `th:each` 与 `th:if` 写在同一元素上会逐次生成重复的 `id`。
- **修复友情链接页的既有缺陷**：分组标题的空判断写在 `th:each` 所在元素上，而 Thymeleaf 先执行 `th:each`，导致判断实际作用于整个列表，空分组仍会渲染出标题。

## v2.1.4 更新

相较于 v2.1.3，本版本重做音乐播放器的音源配置，并修复若干交互与无障碍问题：
...
- 后台的音乐 API 字段补充说明，讲清 Meting 与公开镜像的性质、失效风险，并给出推荐做法。
- **新增顶栏音乐按钮**。浏览器通常只允许有过交互的访客自动播放，首次访问必须手动开始，因此在导航栏右上角提供播放/暂停按钮，图标随真实播放状态切换。
- **新增自动播放开关**（默认关闭）。
- 音乐控件未配置音源时不再渲染，"加载失败"与"未配置"不再混淆。
